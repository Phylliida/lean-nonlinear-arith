...
* `mkBinary` (:1210) — the resultant-composition engine: build the
  ...
  unique. The two exits of the source are DATA (`MkBinaryResult`):
  success, or became-basic — z3 `return mk_basic(a, b, c)` — which the
  caller re-dispatches through its own mixed path (with a basic operand
  the full op's dispatch lands exactly there). This keeps the ops
  non-recursive (design review 2026-07-31, item 4).
...
* The ops (:1584-1883): `add`/`sub`/`mul` dispatch tables verbatim,
  ...
  `power` (`mk_unary`, :1292 — reachable via `t_eval`'s `vm.power`).

**Throw paths are `Option` (design review item 2 — identical behavior
in practice, never a silent wrong value):** z3 *throws* on `inv` of
zero (:1836), `div` by zero (:1869), and `power` `0^0` (:1560) — the
exception unwinds out of the nlsat call. The faithful image is `none`
propagated to the caller, NOT Lean's quiet `1/0 = 0` or `0^0 = 1`.
...
2. `inv`'s convert hitting the exact root: z3 *throws* (:1858), but
   here — unlike the zero-input throws above — the result is perfectly
   defined (the value IS that dyadic). We return `.rat` of it; this can
   only let us continue where z3 crashes, never the reverse (parity of
   verdicts is upward).
...
def setCore (p : QPoly) (ri : MpbqI) (seq : Array QPoly) (lV _uV : Int)
    (minimal : Bool) : RAlg :=
...
/-- The two exits of z3's `mk_binary` (:1272-1288) as data. The
became-basic branches carry the discovered RATIONAL value (so callers
never face an impossible root/root case) and the source's exact restore
state:

* `becameBasicA`: `a` became basic (z3's `||` short-circuit: `b` is NOT
  refined this round) and `b` comes back **over-refined** — z3's
  `saved_b` restore fires only in the destructor AFTER `mk_basic`
  returns, so the caller's re-dispatch must see this interval; the
  caller applies `restoreIfTooSmall b` to what the re-dispatch leaves.
* `becameBasicB`: `b` became basic; `a` has z3's explicit pre-`mk_basic`
  `saved_a.restore_if_too_small` already applied (inside `mkBinary`),
  and the caller returns the post-re-dispatch `a` with NO final restore
  (the explicit call disarms the destructor). -/
inductive MkBinaryResult where
  | ok (c a' b' : RAlg)
  | becameBasicA (q : Rat) (x' : RAlg) (y : RAlg)
  | becameBasicB (x' : RAlg) (q : Rat) (y' : RAlg)
deriving Inhabited
...
exactly as z3's `!refine(a) || !refine(b)`. The Sturm chains are our
`|lc|`-normalized `sturmChain` — positive rescaling preserves variation
counts, so the `V`s are value-identical to z3's `sturm_seq` counts.
`partial`: the loop's termination is the analytic argument of nla-31
(intervals halve; distinct factors' roots separate). -/
partial def mkBinary (mkPoly : QPoly → QPoly → QPoly)
    (mkInterval : MpbqI → MpbqI → MpbqI) (a b : RAlg) : MkBinaryResult :=
  match a, b with
  | .root pa _ _ _, .root pb _ _ _ =>
    ...
    let rec loop (x y : RAlg) (seqs : Array (Option (Array QPoly))) : MkBinaryResult :=
      match x, y with
      | .root _ xa xb _, .root _ ya yb _ =>
        let ri := mkInterval ⟨xa, xb⟩ ⟨ya, yb⟩
        ...
        if numRem == 1 && targetI != seqs'.size then
          .ok (setCore factors[targetI]! ri (seqs'[targetI]!.get!)
                targetLV targetUV fullFact)
               (restoreIfTooSmall a x) (restoreIfTooSmall b y)
        else
          match refine1 x with
          | x₁@(.rat q) => .becameBasicA q x₁ y
          | x₁ =>
            match refine1 y with
            | y₁@(.rat q) => .becameBasicB (restoreIfTooSmall a x₁) q y₁
            | y₁ => loop x₁ y₁ seqs'
      | _, _ =>
        -- unreachable by the mk_binary contract (z3 SASSERTs both
        -- operands algebraic at entry and in the loop)
        .ok x x y
    loop a b seqs0
  | _, _ => .ok a a b   -- unreachable: callers dispatch root/root only

/-- The two exits of z3's `mk_unary` (:1348-1361) as data; the
became-basic branch carries the discovered rational (z3's restore there
is a no-op on the basic cell). -/
inductive MkUnaryResult where
  | ok (c a' : RAlg)
  | becameBasic (q : Rat) (x' : RAlg)
deriving Inhabited

/-- z3 `mk_unary` (:1292): the single-operand analogue of `mkBinary`
(`save_intervals` on `a` only, restore on both exits). `partial`: same
nla-31 termination argument as `mkBinary`. -/
partial def mkUnary (mkPoly : QPoly → QPoly) (mkInterval : MpbqI → MpbqI)
    (a : RAlg) : MkUnaryResult :=
  match a with
  | .root pa _ _ _ =>
    let p := mkPoly pa
    let (fullFact, fs) := factor (QPoly.toZPoly p)
    let factors : Array QPoly := fs.factors.map fun (fp, _) => ZPoly.toQPoly fp
    let seqs0 : Array (Option (Array QPoly)) := factors.map fun f => some (sturmChain f)
    let rec loop (x : RAlg) (seqs : Array (Option (Array QPoly))) : MkUnaryResult :=
      match x with
      | .root _ xa xb _ =>
        let ri := mkInterval ⟨xa, xb⟩
        let (numRem, targetI, targetLV, targetUV, seqs') := Id.run do
          let mut numRem := 0
        ...
        if numRem == 1 && targetI != seqs'.size then
          ...
          match refine1 x with
          | x₁@(.rat q) => .becameBasic q x₁
          | x₁ => loop x₁ seqs'
      | _ => .ok x x   -- unreachable: mk_unary's operand is algebraic
    loop a seqs0
  | _ => .ok a a   -- unreachable: callers dispatch root-only
...
same value). `partial`: the walk's termination is sign-stability near
the endpoint (nla-31). -/
...
/-- The `(basic, ·)` half of z3's `add` dispatch (:1644), including the
zero shortcut — the became-basic re-dispatch target (`mk_basic` is the
full op; with a basic left operand its dispatch lands exactly here). -/
def addRatL (q : Rat) (b : RAlg) : RAlg × RAlg × RAlg :=
  if q == 0 then (b, .rat q, b)
  else match b with
    | .rat qb => (.rat (q + qb), .rat q, b)
    | .root p aa ba m => (addAlgebraicBasic true p aa ba m q, .rat q, b)

/-- The `(·, basic)` half of z3's `add` dispatch. -/
def addRatR (a : RAlg) (q : Rat) : RAlg × RAlg × RAlg :=
  if q == 0 then (a, a, .rat q)
  else match a with
    | .rat qa => (.rat (qa + q), a, .rat q)
    | .root p aa ba m => (addAlgebraicBasic true p aa ba m q, a, .rat q)

/-- The `(basic, ·)` half of z3's `sub` dispatch (:1669): basic−algebraic
goes through the `IsAdd := false` mixed path and a final `neg`
(:1684-1688). -/
def subRatL (q : Rat) (b : RAlg) : RAlg × RAlg × RAlg :=
  if q == 0 then (neg b, .rat q, b)
  else match b with
    | .rat qb => (.rat (q - qb), .rat q, b)
    | .root p aa ba m => (neg (addAlgebraicBasic false p aa ba m q), .rat q, b)

/-- The `(·, basic)` half of z3's `sub` dispatch. -/
def subRatR (a : RAlg) (q : Rat) : RAlg × RAlg × RAlg :=
  if q == 0 then (a, a, .rat q)
  else match a with
    | .rat qa => (.rat (qa - q), a, .rat q)
    | .root p aa ba m => (addAlgebraicBasic false p aa ba m q, a, .rat q)

/-- The `(basic, ·)` half of z3's `mul` dispatch (:1756), zero shortcut
included. -/
def mulRatL (q : Rat) (b : RAlg) : RAlg × RAlg × RAlg :=
  if q == 0 then (.rat 0, .rat q, b)
  else match b with
    | .rat qb => (.rat (q * qb), .rat q, b)
    | .root p aa ba m => (mulAlgebraicBasic p aa ba m q, .rat q, b)

/-- The `(·, basic)` half of z3's `mul` dispatch. -/
def mulRatR (a : RAlg) (q : Rat) : RAlg × RAlg × RAlg :=
  if q == 0 then (.rat 0, a, .rat q)
  else match a with
    | .rat qa => (.rat (qa * q), a, .rat q)
    | .root p aa ba m => (mulAlgebraicBasic p aa ba m q, a, .rat q)

/-- z3 `add(numeral, numeral, c)` (:1644): zero shortcuts, basic+basic,
the mixed translate path, algebraic+algebraic via `mkBinary` with
`resultant_y(pa(x−y), pb(y))`; the became-basic exits re-dispatch
through `addRatL`/`addRatR` (= `mk_basic` with a basic operand). -/
def add (a b : RAlg) : RAlg × RAlg × RAlg :=
  if isZeroV a then (b, a, b)
    ...
    | .root .., .root .. =>
      match mkBinary (fun pa pb => BivPoly.resultantElimY (BivPoly.composeXMinusY pa) pb)
              MpbqI.add a b with
      | .ok c a' b' => (c, a', b')
      | .becameBasicA q _ y =>
        let (c, a'', y'') := addRatL q y
        (c, a'', restoreIfTooSmall b y'')
      | .becameBasicB x' q _ =>
        let (c, a'', y'') := addRatR x' q
        (c, a'', y'')

/-- z3 `sub(numeral, numeral, c)` (:1669): `0 − b = −b`, `a − 0 = a`;
algebraic−algebraic via `mkBinary` with `resultant_y(pa(x+y), pb(y))`
and interval subtraction. -/
def sub (a b : RAlg) : RAlg × RAlg × RAlg :=
  if isZeroV a then (neg b, a, b)
    ...
    | .root .., .root .. =>
      match mkBinary (fun pa pb => BivPoly.resultantElimY (BivPoly.composeXPlusY pa) pb)
              MpbqI.sub a b with
      | .ok c a' b' => (c, a', b')
      | .becameBasicA q _ y =>
        let (c, a'', y'') := subRatL q y
        (c, a'', restoreIfTooSmall b y'')
      | .becameBasicB x' q _ =>
        let (c, a'', y'') := subRatR x' q
        (c, a'', y'')
...
def mul (a b : RAlg) : RAlg × RAlg × RAlg :=
  if isZeroV a || isZeroV b then (.rat 0, a, b)
    ...
    | .root .., .root .. =>
      match mkBinary (fun pa pb => BivPoly.resultantElimY (BivPoly.composeXDivY pa) pb)
              MpbqI.mul a b with
      | .ok c a' b' => (c, a', b')
      | .becameBasicA q _ y =>
        let (c, a'', y'') := mulRatL q y
        (c, a'', restoreIfTooSmall b y'')
      | .becameBasicB x' q _ =>
        let (c, a'', y'') := mulRatR x' q
        (c, a'', y'')
...
to dyadic by `convert_q2bq_interval`. `none` = z3's throw on a zero
input (:1836-1838) — see the file header. On the convert's exact-root
case z3 throws but the value is well-defined — became-rational, see the
file header. -/
def inv : RAlg → Option RAlg
  | .rat q => if q == 0 then none else some (.rat (1 / q))
  | x@(.root ..) =>
    ...
    | .root p a b m =>
      ...
      | .inl r => some (.rat r.toRat)
      | .inr (c, d) => some (mkRoot p' c d m)

/-- z3 `div(numeral, numeral, c)` (:1868): `inv` on a COPY of the divisor
(z3 `set(invb, b); inv(invb)` — `b` itself is never mutated), then `mul`.
`none` = z3's throw on a zero divisor (:1869-1871). -/
def div (a b : RAlg) : Option (RAlg × RAlg × RAlg) :=
  if isZeroV b then none
  else
    match inv b with
    | none => none      -- unreachable given b ≠ 0
    | some binv =>
      let (c, a', _) := mul a binv
      some (c, a', b)

/-- z3 `power(numeral, k, b)` (:1559): `0^0` throws (`none`);
`k = 0 ⇒ 1`, `k = 1 ⇒ a`, `a = 0 ⇒ 0`, basic ⇒ rational power, else
`mkUnary` with `resultant_y(x − y^k, pa(y))` and the interval power.
Returns `(result, refinedA)` (the stored-cell write-back for the
const_cast sites, e.g. `t_eval_core`'s `vm.power(x2v(x), …)`). -/
def power (a : RAlg) (k : Nat) : Option (RAlg × RAlg) :=
  if k == 0 && isZeroV a then none
  else if k == 0 then some (.rat 1, a)
  else if k == 1 then some (a, a)
  else if isZeroV a then some (.rat 0, a)
  else
    match a with
    | .rat q => some (.rat (q ^ k), a)
    | .root .. =>
      match mkUnary (fun pa => BivPoly.resultantElimY (xMinusYPow k) pa)
              (MpbqI.pow · k) a with
      | .ok c a' => some (c, a')
      | .becameBasic q' x' => some (.rat (q' ^ k), x')
