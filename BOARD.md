...
- [nla-14 Slice 4 `done` (2026-08-14) — the `nonlinear_arith` elab + L1/L2 layering + acceptance: sandboxed saturateCore fast path, clean rollback, per-layer withLayerHeartbeats budgets, stats variant, run_cmd counter-sandwich layer pins; probe finding: int1/int2 are L1-closable, B&B-through-layering carried by new x*(x+1)=3 drivers; design review queued](board/nla-14-slice-4-tactic-acceptance.md) `done`
