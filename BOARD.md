...
- [nla-14 plan `done` (2026-08-13 eve → 2026-08-14, all four slices + design reviews landed — the `nonlinear_arith` tactic ships: full Boolean structure via Tseitin proxies (z3's mechanism), SAT-model display, L1/L2 layering with per-layer budgets)](board/nla-14-plan.md) `done`
...
- [nla-14 Slice 2 `done` (2026-08-14) — reify+Tseitin+bridge: the `nla_frontend` transform to the walk's refutation goal; bound-recursion boolDefHolds + boundary lemmas; normNeg polarity normalization (real catch); pins incl. the proxy path walked by hand](board/nla-14-slice-2-reify-tseitin-bridge.md) `done`
- [nla-14 Slice 4 `done` (2026-08-14) — the `nonlinear_arith` elab + L1/L2 layering + acceptance: sandboxed saturateCore fast path, clean rollback, per-layer withLayerHeartbeats budgets, stats variant, run_cmd counter-sandwich layer pins; probe finding: int1/int2 are L1-closable, B&B-through-layering carried by new x*(x+1)=3 drivers](board/nla-14-slice-4-tactic-acceptance.md) `done`
- [nla-14 Slice 4 design review `done` (2026-08-14, Danielle-requested) — R-i REAL (instrumentation entry-semantics + OrchestrateExit, fixed same-day), R-ii REAL (div/mod × nlsat-tier composition gap, boarded to nla-16); nla-14 DONE → nla-15](board/nla-14-slice-4-design-review.md) `done`
