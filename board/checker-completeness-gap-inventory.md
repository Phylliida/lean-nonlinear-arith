...
| G6 | ~~intBranch (integer branch-and-bound)~~ **DONE 2026-08-13 (12e, one session)** — z3 `search_check` B&B loop ported (scan/tighten/restart/emission); payload `(x, lo : Int)` (decision 2); checker discharge from context integrality (decision 1); `isV0 := !isS1Gated` — the last shape gate lifted; int1 (two-round trace) walked end-to-end | landed in Solver/RAlg/Refute/Walk | — |
